...
function connectUsers($party1,$party2,$connectMsg = ""){
global $connection;
   $cid = uid(34);
   $mid = ($connectMsg == "")?0:1;
   $query = "INSERT INTO `connections`(`party1`, `party2`, `cid`, `last_mid`, `last_active`) VALUES ('$party1','$party2','$cid','$mid','strtotime()')";
   $message = userData($party1)['fullname']." initiated a chat.";
  if($connectMsg != ""){
   $query1 = "INSERT INTO `messages`(`list_id`, `mid`, `user_id`, `message`, `seen`) VALUES ('$cid','1','bot','$connectMsg','0')";
 ...
 */
 ...
 $query1 = "SELECT * FROM `messages` WHERE `list_id`='".$list_id."' && `mid` >= ".$key." ORDER BY mid ASC";    
